...
- `CHANNEL_ID` — например `@asgardchanel` (если публикация включена)
...
Если нужно только готовить новости без отправки в канал:

1) В `config.yaml` установите `app.publish_enabled: false`
2) `CHANNEL_ID` можно оставить пустым

Бот продолжит готовить тексты и картинки и присылать их на утверждение.
